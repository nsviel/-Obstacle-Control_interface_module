...
def post_lidar_start(who):
    send_command("/lidar_start", "[#] False alarm sended")

def post_lidar_stop(who):
    send_command("/lidar_stop", "[#] False alarm sended")
...
def post_new_param_py(lvl1, lvl2, value):
    connected = param_co.state_co["hubium"]["connected"]
    ip = param_co.state_co["hubium"]["ip"]
    port = param_co.state_co["hubium"]["http_server_port"]
    header = {"Content-type": "application/json"}
    ...
    if(connected):
        try:
            sock = client.HTTPConnection(ip, port, timeout=1)
            sock.request("POST", "/new_param_py", file, header)
            sock.close()
        except:
            print("[error] Sending new state py failed for ip: %s | port: %d"% (ip, port))
