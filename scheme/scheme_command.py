...
def command_lidar_source():
    ...
    http_client_post.post_param_val("hu", "self", "sock_server_l1_source", source_l1)
    http_client_post.post_param_val("hu", "self", "sock_server_l2_source", source_l2)
...
def command_comboip():
    ...
    if(py_ip != None):
        ...
        http_client_post.post_param_val("hu", "pywardium", "ip", py_ip)
    if(ed_ip != None):
        param_co.state_hu["edge"]["ip"] = ed_ip
        dpg.set_value("ed_ip", ed_ip)
        http_client_post.post_param_val("hu", "edge", "ip", ed_ip)
    if(sncf_ip != None):
        param_co.state_hu["sncf"]["broker_ip"] = sncf_ip
        dpg.set_value("sncf_ip", sncf_ip)
        http_client_post.post_param_val("hu", "sncf", "broker_ip", sncf_ip)
