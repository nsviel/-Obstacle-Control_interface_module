...
def test_ssd_con():
    path = param_interface.path_ssd
    if(os.path.exists(path) and path.find("lidar_ssd") != -1):
        hdd = psutil.disk_usage(param_interface.path_ssd)
        param_interface.state_interface["ssd"]["connected"] = True
        param_interface.state_interface["ssd"]["space_used"] = int(hdd.used / (2**30))
        param_interface.state_interface["ssd"]["space_total"] = int(hdd.total / (2**30))
    else:
        param_interface.state_interface["ssd"]["connected"] = False
        param_interface.state_interface["ssd"]["space_used"] = 0
...
def determine_path():
    ...
    param_interface.state_interface["path"]["dir_l2"] = os.path.join(param_interface.state_interface["path"]["dir_capture"], "lidar_2")
    param_interface.state_interface["path"]["path_l1_file"] = os.path.join(param_interface.state_interface["path"]["dir_l1"], param_interface.state_interface["path"]["file_name"])
...
def check_directories():
    ...
    if(connected):
        # Capture directory
        path = param_interface.state_interface["path"]["dir_capture"]
        if(os.path.exists(path) == False):
            create_directory(path)
        # Lidar 1 directory
        path = param_interface.state_interface["path"]["dir_l1"]
        if(os.path.exists(path) == False):
            create_directory(path)
        # Lidar 2 directory
        path = param_interface.state_interface["path"]["dir_l2"]
        if(os.path.exists(path) == False):
            create_directory(path)
