...
def init_image():
    try:
        data_w, data_h, data_c, data_i = dpg.load_image(param_interface.path_image_empty)

        computer_w, computer_h, computer_c, computer_i = dpg.load_image(param_interface.path_icon_computer)
        wifi_w, wifi_h, wifi_c, wifi_i = dpg.load_image(param_interface.path_icon_wifi)
        lidar_w, lidar_h, lidar_c, lidar_i = dpg.load_image(param_interface.path_icon_lidar)
        server_w, server_h, server_c, server_i = dpg.load_image(param_interface.path_icon_server)
        train_w, train_h, train_c, train_i = dpg.load_image(param_interface.path_icon_train)
        cloud_w, cloud_h, cloud_c, cloud_i = dpg.load_image(param_interface.path_icon_cloud)
        soft_w, soft_h, soft_c, soft_i = dpg.load_image(param_interface.path_icon_soft)
        ssd_w, ssd_h, ssd_c, ssd_i = dpg.load_image(param_interface.path_icon_ssd)
        database_w, database_h, database_c, database_i = dpg.load_image(param_interface.path_icon_database)

        param_interface.image_w = data_w
        param_interface.image_h = data_h
