...
class Link:
    ...
    def update(self):
        colorization.colorize_link_http(param_control.state_edge["hub"]["interface"]["slam_http_connected"], self.link_http_hub_slam)
        colorization.colorize_link_http(param_control.state_edge["hub"]["interface"]["ai_http_connected"], self.link_http_hub_ai)
        colorization.colorize_link_http(param_control.state_edge["hub"]["interface"]["capture_http_connected"], self.link_http_cap_hub)
        colorization.colorize_link_http(param_control.state_edge["hub"]["interface"]["operator_broker_connected"], self.link_mqtt_hub_operator)

        colorization.colorize_link_socket(param_control.state_edge["hub"]["interface"]["slam_sock_connected"], self.link_sock_hub_slam)
    ...
    def update_dependencies(self):
        ...
        if(param_control.state_control["control"]["interface"]["edge_http_connected"]):
            ...
            if(param_control.state_edge["hub"]["interface"]["ai_http_connected"]):
                param_control.state_edge["ai"]["info"]["status"] = "Online"
            if(param_control.state_edge["hub"]["interface"]["slam_http_connected"]):
        ...
        if(param_control.state_edge["hub"]["info"]["status"] == "Offline"):
            ...
            param_control.state_edge["hub"]["interface"]["ai_http_connected"] = False
            param_control.state_edge["hub"]["interface"]["slam_sock_connected"] = False
            param_control.state_edge["hub"]["interface"]["slam_http_connected"] = False
